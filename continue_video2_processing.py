...
class ContinueVideo2Processor:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        # 设置输出目录
        self.base_output_dir = "data_Video_20250821140339629"
        ...
        # 创建所有目录
        for dir_path in [self.spots_dir, self.burrs_dir, self.texture_dir, self.analysis_dir]:
        ...
        try:
            ...
            for i, roi_path in enumerate(tqdm(roi_files, desc="毛刺检测")):
                try:
                    ...
                    # 获取毛刺二值掩码
                    burr_binary = burr_result.get('all_burr_binary_mask', None)
    ...
    def create_combined_video(self, fps: float = 2.39) -> Dict[str, Any]:
        ...
        try:
            ...
            # 设置视频编码器
            output_video_path = os.path.join(self.base_output_dir, 'combined_video.mp4')
            ...
            for i, roi_path in enumerate(tqdm(roi_files, desc="创建组合视频")):
                try:
                    ...
                    if len(name_parts) >= 2:
                        ...
                        # 添加标签
                        combined_frame = self.add_labels_to_combined_frame(combined_frame, roi_width)
    ...
    def add_labels_to_combined_frame(self, combined_frame: np.ndarray, section_width: int) -> np.ndarray:
        """
        为组合帧添加标签
        """
        try:
            ...
            # 转换回OpenCV格式
            labeled_frame = cv2.cvtColor(np.array(pil_image), cv2.COLOR_RGB2BGR)
    ...
    def run_remaining_steps(self) -> Dict[str, Any]:
        """
        运行剩余的处理步骤
        ...
        # 步骤1: 斑块分析
        print("\n步骤1: 斑块分析...")
        result1 = self.generate_spot_analysis()
        results['spot_analysis'] = result1
        ...
        # 步骤2: 毛刺分析
        print("\n步骤2: 毛刺分析...")
        result2 = self.generate_burr_analysis()
        results['burr_analysis'] = result2
        
        # 步骤3: LBP纹理分析
        print("\n步骤3: LBP纹理分析...")
        result3 = self.generate_texture_analysis()
        results['texture_analysis'] = result3
        
        # 步骤4: 时间序列图表
        print("\n步骤4: 生成时间序列图表...")
        result4 = self.generate_temporal_plots()
        results['temporal_plots'] = result4
        
        # 步骤5: 组合视频
        print("\n步骤5: 创建组合视频...")
        result5 = self.create_combined_video(fps=2.39)
        results['combined_video'] = result5
...
def main():
    """主函数"""
    # 初始化处理器
    processor = ContinueVideo2Processor()
    
    # 运行剩余步骤
    results = processor.run_remaining_steps()
