...
class BurrExtractor:
    ...
    def __init__(self, config: Dict[str, Any] = None, base_output_dir: str = None):
        ...
        self.burr_processor = BurrProcessor(self.config)
        ...
        self.burr_dir = os.path.join(self.base_output_dir, 'burr_imgs')
        
        # 创建毛刺输出目录
        os.makedirs(self.burr_dir, exist_ok=True)
    
    def extract_burrs_from_roi_images(self, roi_dir: str, burr_dir: str) -> Dict[str, Any]:
        """
        从ROI图像目录提取毛刺特征
        ...
        Returns:
            毛刺提取结果统计
        """
        print(f"=== 从ROI图像提取毛刺特征 ===")
        print(f"输入ROI目录: {roi_dir}")
        print(f"输出毛刺目录: {burr_dir}")
        ...
        try:
            ...
            # 创建输出目录
            os.makedirs(burr_dir, exist_ok=True)
            ...
            for i, roi_path in enumerate(tqdm(roi_files, desc="提取毛刺特征")):
                try:
                    # 从文件名提取帧号
                    ...
                    # 生成输出文件名
                    filename = f"frame_{frame_number:06d}_burr.png"
                    output_file = os.path.join(burr_dir, filename)
                    ...
                    # 处理单个ROI毛刺检测
                    result = self.burr_processor.process_single_roi_burrs(roi_path, output_file)
            ...
            return {'success': False, 'error': f'毛刺特征提取时出错: {str(e)}'}
    
    def extract_burrs_from_all_rois(self) -> Dict[str, Any]:
        """
        从所有ROI图像提取毛刺特征
        ...
        try:
            ...
            for i, roi_path in enumerate(tqdm(roi_files, desc="提取毛刺特征")):
                try:
                    ...
                    burr_output_path = os.path.join(self.burr_dir, f"{name}_burr.png")
                    
                    # 检查毛刺文件是否已存在，如果存在则跳过
                    if os.path.exists(burr_output_path):
                        success_count += 1
                        continue
                    
                    # 处理单个ROI毛刺检测
                    result = self.burr_processor.process_single_roi_burrs(roi_path, burr_output_path)
            ...
            return {
                ...
                'output_dir': self.burr_dir
            }
            
        except Exception as e:
            return {'success': False, 'error': f'毛刺特征提取时出错: {str(e)}'}
    
    def continue_burr_extraction(self) -> Dict[str, Any]:
        """
        继续完成毛刺特征提取（跳过已存在的文件）
        ...
        try:
            ...
            for i, roi_path in enumerate(tqdm(roi_files, desc="继续毛刺特征提取")):
                try:
                    # 生成输出文件名
                    basename = os.path.basename(roi_path)
                    name, ext = os.path.splitext(basename)
            ...
            return {'success': False, 'error': f'继续毛刺特征提取时出错: {str(e)}'}
    ...
    def get_extraction_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 获取所有毛刺文件
            burr_pattern = os.path.join(self.burr_dir, "*_burr.png")
            burr_files = glob.glob(burr_pattern)
            
            total_rois = len(roi_files)
            total_burrs = len(burr_files)
            completion_rate = (total_burrs / total_rois * 100) if total_rois > 0 else 0
            
            return {
                'success': True,
                'total_rois': total_rois,
                'total_burrs': total_burrs,
                'completion_rate': completion_rate,
                'roi_dir': self.roi_dir,
                'burr_dir': self.burr_dir
            }
...
def main():
    ...
    default_burr_dir = "data_video_20250821152112032/burr_imgs"
    ...
    if len(sys.argv) >= 3:
        ...
        roi_dir = sys.argv[1]
        burr_dir = default_burr_dir
        print(f"使用默认输出目录: {burr_dir}")
    else:
    ...
    # 判断burr_dir是否存在
    if not os.path.exists(burr_dir):
        os.makedirs(burr_dir, exist_ok=True)
    
    # 初始化毛刺提取器
    extractor = BurrExtractor()
    
    # 执行毛刺特征提取
    result = extractor.extract_burrs_from_roi_images(
        roi_dir=roi_dir, 
        burr_dir=burr_dir
    )
