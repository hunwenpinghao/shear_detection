...
class TearTextureDensityAnalyzer:
    ...
    def recompute_texture_from_existing_regions(self, image_files, step2_dir):
        """从现有的撕裂面区域文件重新计算纹理特征（快速模式）"""
        
        # 加载现有的撕裂面区域数据
        existing_regions = self.load_existing_tear_regions(image_files, step2_dir)
        
        if not existing_regions:
            print("未找到现有的撕裂面区域文件，尝试从mask文件加载...")
            return self.recompute_texture_from_existing_masks(image_files, 
                os.path.join(os.path.dirname(step2_dir), 'step1_tear_masks'), step2_dir)
        
        # 从撕裂面区域计算纹理特征
        print("从现有撕裂面区域重新计算纹理特征...")
